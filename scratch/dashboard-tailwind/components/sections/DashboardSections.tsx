  ...
  return (
    <aside className="w-full md:w-64 md:h-screen md:sticky md:top-0 bg-slate-900 text-slate-300 flex-shrink-0 border-r border-slate-800 flex flex-col">
      <div className="p-6 border-b border-slate-800 flex items-center gap-2">
        <span className="h-6 w-6 rounded bg-indigo-500 flex items-center justify-center text-white font-bold text-xs">
          D
        </span>
      ...
      <nav className="p-4 space-y-1 flex-1">
        {['Overview', 'Analytics', 'Users', 'Settings'].map((item, idx) => (
          <a
            key={item}
            href="#"
            className={`flex items-center gap-3 px-3 py-2 text-sm rounded-lg font-medium transition-colors ${idx === 0 ? 'bg-slate-800 text-white' : 'hover:bg-slate-800/50 hover:text-white'}`}
          >
            {item}
          </a>
        ))}
  ...
  return (
    <header className="flex flex-col justify-between gap-4 border-b border-slate-200 bg-white px-6 py-4 md:flex-row md:items-center w-full">
      ...
      <div className="flex gap-2">
        <button className="rounded-lg border border-slate-200 bg-white hover:bg-slate-50 px-3.5 py-1.5 text-xs font-semibold text-slate-600 transition-colors">
          Export
        </button>
        <button className="rounded-lg bg-indigo-600 hover:bg-indigo-500 px-3.5 py-1.5 text-xs font-semibold text-white transition-colors shadow-sm shadow-indigo-600/10">
          Create report
        </button>
      </div>
    </header>
      ...
      {dashboardData.stats.map((stat) => (
        <DashboardCard key={stat.label} title={stat.label}>
          <div className="flex justify-between items-baseline">
            ...
            <span
              className={`text-xs font-bold px-1.5 py-0.5 rounded ${stat.trend === 'up' ? 'text-emerald-600 bg-emerald-50' : 'text-rose-600 bg-rose-50'}`}
            >
              {stat.change}
            </span>
          </div>
          ...
          <div className="flex h-48 items-end gap-3 pt-6 border-b border-slate-100">
            {dashboardData.chart.map((value, index) => (
              <div key={index} className="flex-1 flex flex-col items-center">
                <div
                  className="w-full rounded-t bg-indigo-500 hover:bg-indigo-400 transition-colors"
        ...
        <DashboardCard title="Recent Activity">
          <div className="space-y-3">
            {dashboardData.activity.map((item, idx) => (
              <div
                key={idx}
                className="flex flex-col gap-1 border-b border-slate-50 pb-2 last:border-0 last:pb-0"
              >
            ...
            <tbody>
              {dashboardData.rows.map((row) => (
                ...
                >
                  ...
                  <td>
                    <span
                      className={`px-2 py-0.5 rounded text-[10px] font-semibold ${row.status === 'Healthy' ? 'bg-emerald-50 text-emerald-700' : row.status === 'Review' ? 'bg-amber-50 text-amber-700' : 'bg-indigo-50 text-indigo-700'}`}
                    >
                      {row.status}
                    </span>
                  </td>
