...
export class NpmWorkspaceAdapter implements FullstackAdapter {
  ...
  contribute(context: FullstackGenerationContext): FullstackContribution {
    ...
    if (context.config.tools.eslint) {
      rootScripts.lint = 'npm run lint --workspaces --if-present';
    }
    if (context.config.language === 'typescript') {
      rootScripts.typecheck = 'npm run typecheck --workspaces --if-present';
    }
...
export class FallbackStarterAdapter implements FullstackAdapter {
  id = 'fallback-starter';
  ...
  contribute(context: FullstackGenerationContext): FullstackContribution {
    ...
    const files: FullstackGeneratedFile[] = [
      {
        path: 'README.md',
        content: `# ${projectName}\n\nGenerated with Structify v1.3.2. Start by reviewing the frontend and backend workspaces.\n`,
        source: 'fallback-starter',
        owner: 'fallback-starter',
      },
      {
        path: '.env.example',
