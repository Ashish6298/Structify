...
export const INTEGRATIONS_CATALOG: Integration[] = [
  // --- AUTHENTICATION ---
  {
    ...
    category: 'auth',
    compatibility: {
      frontends: ['next', 'vite-react'],
      backends: ['express', 'nest', 'none', 'unknown'],
    },
  ...
  },
  {
    ...
    envVars: [
      {
        key: 'NEXTAUTH_SECRET',
        value: 'nextauth-secret-key',
        description: 'Encryption secret for NextAuth sessions',
      },
    ],
    ...
    installSteps: [
      ...
      'Add NEXTAUTH_SECRET env variable',
    ],
  },
  {
    id: 'clerk',
  ...
  },
  {
    ...
    installSteps: [
      ...
      'Configure lib/supabase.ts client',
      'Add Supabase environment configs',
    ],
  },
  {
    id: 'firebase-auth',
    ...
    envVars: [
      {
        ...
        value: 'api-key-here',
        description: 'Firebase Web SDK API Key',
      },
    ],
    docsLink: 'https://firebase.google.com/docs/auth',
    installSteps: [
      'Install firebase package',
      'Configure lib/firebase.ts',
  ...
  // --- PAYMENTS ---
  {
    ...
    installSteps: [
      'Install stripe SDKs',
      'Configure stripe client lib/stripe.ts',
      'Add Stripe Secret API Key',
    ],
  },
  {
    id: 'lemon-squeezy',
    ...
    generatedFiles: [
      {
        ...
        content: `import { lemonSqueezySetup } from "@lemonsqueezy/lemonsqueezy.js";\nlemonSqueezySetup({ apiKey: process.env.LEMON_SQUEEZY_API_KEY });\n`,
      },
    ],
    envVars: [
    ...
    installSteps: [
      'Install lemonsqueezy.js client sdk',
      'Configure lemonsqueezy client lib/lemonsqueezy.ts',
      'Add API Key',
    ],
  },
  ...
  // --- MONITORING ---
  {
    ...
    installSteps: [
      ...
      'Create client config file sentry.client.config.js',
      'Add Sentry DSN',
    ],
  },
    ...
    envVars: [
      {
        ...
        description: 'Redis Connection Connection URL',
      },
    ],
    docsLink: 'https://redis.io',
    installSteps: [
      'Install ioredis client package',
      'Configure lib/redis.ts connector client',
      'Add REDIS_URL',
    ],
  },
  ...
  // --- EMAIL ---
  {
    ...
    envVars: [{ key: 'RESEND_API_KEY', value: 're_...', description: 'Resend API Key' }],
    docsLink: 'https://resend.com',
    installSteps: [
      'Install resend client package',
      'Configure client lib/email.ts',
      'Add RESEND_API_KEY env',
    ],
  },
  ...
  // --- STORAGE ---
  {
    ...
    envVars: [{ key: 'AWS_REGION', value: 'us-east-1', description: 'AWS S3 region' }],
    docsLink: 'https://aws.amazon.com/s3/',
    installSteps: [
      'Install S3 client package SDK',
      'Configure lib/s3.ts client',
  ...
  // --- ANALYTICS ---
  {
    ...
    installSteps: [
      'Install posthog-js SDK client',
      'Create init module lib/posthog.ts',
      'Add token config to environment',
    ],
  },
];
...
export function getIntegrationsForCategory(category: string, stack: DetectedStack): Integration[] {
  ...
  return INTEGRATIONS_CATALOG.filter((integration) => {
    ...
    const frontendMatch =
      integration.compatibility.frontends.includes(stack.frontend) ||
      integration.compatibility.frontends.includes('unknown');

    // Check compatibility: matching backend framework
    const backendMatch =
      integration.compatibility.backends.includes(stack.backend) ||
  ...
  if (fs.existsSync(absoluteGraphPath)) {
    try {
      ...
      const nodeExists = graph.nodes.some(
        (n: { id: string }) => n.id === `module:${integration.id}`,
      );
      if (!nodeExists) {
        graph.nodes.push({
