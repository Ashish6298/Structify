...
const SETUP_WORKSPACE_CMDS = new Set([
  'init',
  'generate',
  'add',
  'repair',
...
const TEMPLATING_ARTIFACT_CMDS = new Set([
  'templates',
  'generators',
  'validate-template',
  'explain-template',
...
const REGISTRY_PACKAGE_CMDS = new Set([
  'registry',
  'install',
  'uninstall',
  'update',
...
const GRAPH_VISUALIZATION_CMDS = new Set([
  'dependency-graph',
  'template-graph',
  'blueprint-graph',
  'plugin-graph',
]);
const PERFORMANCE_REPORT_CMDS = new Set([
  'workspace-report',
  'export-report',
  'profile',
  'benchmark',
...
const DIAGNOSTIC_EXPLAIN_CMDS = new Set([
  'validate-workspace',
  'diagnose',
  'explain-generation',
  'explain-merge',
...
function formatHelpScreen(program: Command, context: ReturnType<typeof createCLIContext>): string {
  ...
  // 1. Usage
  lines.push(
    `${cyan('◆')} ${bold('Usage:')} ${purple('structify')} ${gray('[options]')} ${cyan('[command]')}`,
  );
  lines.push('');

  // 3. Global Options
  lines.push(
    `${gray('┌─')} ${bold('GLOBAL OPTIONS')} ${gray('───────────────────────────────────────┐')}`,
  );
  lines.push(
    `${gray('│')} ${cyan('-V, --version')}     ${gray('Output the version number')}              ${gray('│')}`,
  );
  lines.push(
    `${gray('│')} ${cyan('--verbose')}         ${gray('Print additional diagnostic logs')}       ${gray('│')}`,
  );
  lines.push(
    `${gray('│')} ${cyan('--debug')}           ${gray('Enable debug output and stack traces')}    ${gray('│')}`,
  );
  lines.push(
    `${gray('│')} ${cyan('--json')}            ${gray('Render machine-readable JSON payloads')}   ${gray('│')}`,
  );
  lines.push(
    `${gray('│')} ${cyan('--no-color')}        ${gray('Omit colored console outputs')}            ${gray('│')}`,
  );
  lines.push(
    `${gray('│')} ${cyan('--cwd <path>')}     ${gray('Change context working directory')}        ${gray('│')}`,
  );
  lines.push(gray('└────────────────────────────────────────────────────────┘'));
  lines.push('');
  ...
  if (setupGroup.length > 0) {
    lines.push(
      `${gray('╭─')} ${bold('SETUP & WORKSPACE')} ${gray('────────────────────────────────────╮')}`,
    );
  ...
  if (intelligenceGroup.length > 0) {
    lines.push(
      `${gray('╭─')} ${bold('INTELLIGENCE & AUDITING')} ${gray('──────────────────────────────╮')}`,
    );
  ...
  if (templatingGroup.length > 0) {
    lines.push(
      `${gray('╭─')} ${bold('TEMPLATING & ARTIFACTS')} ${gray('───────────────────────────────╮')}`,
    );
  ...
  if (registryGroup.length > 0) {
    lines.push(
      `${gray('╭─')} ${bold('REGISTRY & PACKAGES')} ${gray('──────────────────────────────────╮')}`,
    );
  ...
  if (graphGroup.length > 0) {
    lines.push(
      `${gray('╭─')} ${bold('VISUALIZATION & GRAPHS')} ${gray('───────────────────────────────╮')}`,
    );
  ...
  if (migrationGroup.length > 0) {
    lines.push(
      `${gray('╭─')} ${bold('STATE & MIGRATIONS')} ${gray('───────────────────────────────────╮')}`,
    );
  ...
  if (performanceGroup.length > 0) {
    lines.push(
      `${gray('╭─')} ${bold('PERFORMANCE & REPORTS')} ${gray('────────────────────────────────╮')}`,
    );
  ...
  if (diagnosticGroup.length > 0) {
    lines.push(
      `${gray('╭─')} ${bold('EXPLANATIONS & DIAGNOSTICS')} ${gray('───────────────────────────╮')}`,
    );
  ...
  lines.push(
    `${gray('Run')} ${purple('structify help <command>')} ${gray('for custom command parameters')}`,
  );
  lines.push('');
